...
Description: Modal UI module for 2x speed purchases
Interacts With:
  - SpeedTransformationHandler: Server-side purchase and speed modification
...
local NotificationManager = require(script.Parent.Parent.Notifications.NotificationManager)

local SpeedBuyModal = {}

-- Configuration
local SPEED_PRODUCT_ID = 3283965166

-- Get the SpeedPurchaseEvent
...
PurchaseSuccessEvent.Name = "SpeedPurchaseSuccess"
PurchaseSuccessEvent.Parent = script

function SpeedBuyModal.new()
    local player = Players.LocalPlayer
    local playerGui = player:WaitForChild("PlayerGui")
    local purchaseSuccessful = false
    
    print("SpeedBuyModal: Creating new modal instance")
    
    -- Create UI
    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "SpeedBuyModal"
    ...
    purchaseButton.Text = "Purchase for 19 R$"
    ...
    local function showModal()
        print("SpeedBuyModal: Showing modal for", player.Name)
    ...
    local function hideModal()
        print("SpeedBuyModal: Hiding modal")
        ...
        fadeOut.Completed:Connect(function()
            ...
            if purchaseSuccessful then
                print("SpeedBuyModal: Purchase was successful, firing success event")
    ...
    local function onPurchaseSuccess()
        print("SpeedBuyModal: Purchase successful for", player.Name)
        print("SpeedBuyModal: Firing SpeedPurchaseEvent")
        SpeedPurchaseEvent:FireServer()
        purchaseSuccessful = true
        print("SpeedBuyModal: Set purchaseSuccessful to true")
        hideModal()
    end

    -- Connect button events
    purchaseButton.MouseButton1Click:Connect(function()
        print("SpeedBuyModal: Purchase button clicked by", player.Name)
        hideModal()  -- Hide our modal first
        MarketplaceService:PromptProductPurchase(player, SPEED_PRODUCT_ID)
    end)
    ...
    MarketplaceService.PromptProductPurchaseFinished:Connect(function(userId, productId, isPurchased)
        print("SpeedBuyModal: Purchase finished - userId:", userId, "productId:", productId, "isPurchased:", isPurchased)
        if userId == player.UserId and productId == SPEED_PRODUCT_ID and isPurchased then
            onPurchaseSuccess()
        end
    end)
